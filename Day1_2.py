...
input_data: str = '''nqninenmvnpsz874
...
for line in input_data.split('\n'):
    combined_number: list = [[len(line), ''],[0, '']]
    for number in str_to_digit.keys():
        if line.find(number) != -1:
            if line.find(number) <= combined_number[0][0]:
                combined_number[0] = [line.find(number), str_to_digit[number]]
            if line.rfind(number) >= combined_number[1][0]:
                combined_number[1] = [line.rfind(number), str_to_digit[number]]
    for digit in line:
        if digit.isdigit():
            if line.find(digit) <= combined_number[0][0]:
                combined_number[0] = [line.find(digit), digit]
            if line.rfind(digit) >= combined_number[1][0]:
                combined_number[1] = [line.rfind(digit), digit]
    if f'{combined_number[0][1]}{combined_number[1][1]}':
        output += int(f'{combined_number[0][1]}{combined_number[1][1]}')
