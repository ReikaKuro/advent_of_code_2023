...
input_data: str = '''Game 1: 10 green, 9 blue, 1 red; 1 red, 7 green; 11 green, 6 blue; 8 blue, 12 green
...
for game in input_data.split('\n'):
    ...
    for gem in game.replace(',', ';').split(';'):
        gem_number = re.match(r'.*\s(\d+)\s(red|green|blue).*', gem)
        if int(gem_number[1]) > gem_amount[gem_number[2]]:
            gem_amount[gem_number[2]] = int(gem_number[1])
    output += (gem_amount['green']*gem_amount['blue']*gem_amount['red'])
