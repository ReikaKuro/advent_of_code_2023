...
row_amount = len(engine_schematic)
column_amount = len(engine_schematic[0]) - 1


def complete_the_number(num_pos_x, num_pos_y, number) -> list[int, str]:
    completed_number: str = number
    # num_pos_x + num_pos_x... + num_pos_y
    completed_num_pos: str = str(num_pos_x)

    if num_pos_x != 0:
        for index in range(1, num_pos_x + 1):
            character = engine_schematic[num_pos_y][num_pos_x - index]
            completed_number = f'{character}{completed_number}' if character.isdigit() else completed_number
            completed_num_pos = f'{num_pos_x - index}{completed_num_pos}' if character.isdigit() else completed_num_pos
            if not character.isdigit():
                break

    if num_pos_x != column_amount:
        for index in range(num_pos_x + 1, column_amount + 1):
            character = engine_schematic[num_pos_y][index]
            completed_number = f'{completed_number}{character}' if character.isdigit() else completed_number
            completed_num_pos = f'{completed_num_pos}{index}' if character.isdigit() else completed_num_pos
            if not character.isdigit():
                break
    completed_num_pos += f'{num_pos_y}'

    # print(f'Found this number {completed_number}')
    return [int(completed_number), completed_num_pos]


def find_numbers_around_gear(gear_pos_x, gear_pos_y) -> list[list[int, int, str]]:
    numbers_positions: list = []
    min_row = gear_pos_y - 1 if gear_pos_y != 0 else gear_pos_y
    max_row = gear_pos_y + 1 if gear_pos_y != row_amount else gear_pos_y
    min_column = gear_pos_x - 1 if gear_pos_x != 0 else gear_pos_x
    max_column = gear_pos_x + 1 if gear_pos_x != column_amount else gear_pos_x

    for row in range(min_row, max_row + 1):
        for column in range(min_column, max_column + 1):
            if row != gear_pos_y or column != gear_pos_x:
                numbers_positions.append([column, row, engine_schematic[row][column]]) if engine_schematic[row][column].isdigit() else 0
...
for row_number, row in enumerate(engine_schematic):
    ...
    for column_number, column in enumerate(row):
        if column == '*':
            position_x = column_number
            position_y = row_number
            # print(f'Found an * on positions {position_x, position_y}')
            numbers_positions = find_numbers_around_gear(position_x, position_y)
            # print(f'Looking for all number for: {numbers_positions}')
            for pos_x, pos_y, num in numbers_positions:
                completed_num, completed_num_pos = complete_the_number(pos_x, pos_y, num)
                numbers[completed_num_pos] = completed_num

            multiplied_numbers: int = 1
            if 1 < len(numbers) < 3:
                for number in numbers.keys():
                    multiplied_numbers = multiplied_numbers * numbers[number]
                print(f'----->Result for {numbers} is {multiplied_numbers}')
                output += multiplied_numbers
            numbers: dict = {}
